...
from lpcvc.loss import OhemCELoss2D,CrossEntropyLoss, \
      CriterionPixelWise, CriterionPairWiseforWholeFeatAfterPool
...
class kd_senet_stage3_fpn14_b111(nn.Module):
    def __init__(self,
                 nclass=14,
                 backbone='resnet18',
        ...
        super(kd_senet_stage3_fpn14_b111, self).__init__()
        ...
        # in, mid, out
        self.ffm_16 = SEBlockFusionModule(256*self.expansion,256,128,norm_layer=norm_layer)
        self.ffm_8 = SEBlockFusionModule(128*self.expansion,256,128,norm_layer=norm_layer)
        self.ffm_4 = SEBlockFusionModule(64*self.expansion,256,128,norm_layer=norm_layer)
        
        self.clslayer_16 = FPNOutputForSE(128, 64, nclass,norm_layer=norm_layer)
        self.clslayer_4 = FPNOutputForSE(128, 64, nclass,norm_layer=norm_layer)
    ...
    def forward(self, x, lbl=None, t_pred=None, t_feat=None):
        ...
        feat4, feat8, feat16 = self.resnet(x)
        ...
        if self.training:
            ...
            ce_loss = self.sg_loss(outputs,lbl) +  \
                0.5*self.sg_loss(auxout_1, lbl) + \
                    0.5*self.sg_loss(auxout_2,lbl)
    ...
    def get_params(self):
        ...
        for name, child in self.named_children():
            if isinstance(child,(OhemCELoss2D,CrossEntropyLoss, 
                                 CriterionPixelWise, CriterionPairWiseforWholeFeatAfterPool)):
                continue
            elif isinstance(child, (SEBlockFusionModule, FPNOutputForSE)): # not in FPNout
